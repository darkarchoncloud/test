...
public class layerbylayer {
  ...
  private static void preOrder(TreeNode root, StringBuilder preOrder) {
	...
	public ArrayList<ArrayList<Integer>> printLayer(TreeNode root) {
		...
		while (!queue.isEmpty()) {
			int size = queue.size();
			ArrayList<Integer> tempVal = new ArrayList<Integer>(size);
			for (int i = 0; i < size; i++) {
			TreeNode temp = queue.pollFirst();
			
			tempVal.add(temp.key);
	...
	public static void main(String[] args) {
		layerbylayer dark = new layerbylayer();
		TreeNode root = null;
		root = TreeNode.reConstruct(new String[] {"5", "3", "1", "#", "#", "4", "#", "#", "8", "#", "11", "#", "#"});
		ArrayList<ArrayList<Integer>> result = new ArrayList<ArrayList<Integer>>();
		result = dark.printLayer(root);
